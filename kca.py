...
def download_kernel(app, dest_dir, kernel, kernel_size):
    ...
    if os.path.exists(kernel_path):
        if len(open(kernel_path, "rb").read()) != kernel_size:
            ...
            app.headers.update(
                {"Range": "bytes=%s-%s" % (os.path.getsize(kernel_path), kernel_size)}
            )
    ...
    if do_download:
        ...
        kernel_ddeb = requests.get(
            "{}/{}".format(app.k_url, kernel), headers=app.headers, stream=True
        )

        bar_width = 20
        progress = "#" * int(os.path.getsize(kernel_path) / (kernel_size / bar_width))
        bar_template = "%(label)s [{}%(bar)s] %(info)s".format(progress)

        progress_width = bar_width - int(
            os.path.getsize(kernel_path) / (kernel_size / bar_width)
        )
        with click.progressbar(
            kernel_ddeb.iter_content(1024),
...
def unpack_debug_kernel(app, dest_dir, kver, kernel):
    ...
    subprocess.run(
        [
            "ar",
            "x",
            "{}/{}".format(dest_dir, kernel),
            "{}/data.tar.xz".format(dest_dir),
        ],
        check=True,
    )
    ...
    subprocess.run(
        ["tar", "xvf", "data.tar.xz", "-C", dest_dir, kernel_path], check=True
    )
    ...
    with open("debug.S", "wb", 0) as out:
        subprocess.run(
            [
                ...
                "-l",
                "-d",
                "{}/usr/lib/debug/boot/vmlinux-{}".format(dest_dir, kver),
            ],
            stdout=out,
            check=True,
        )
...
@click.option(
    ...
    "--arch",
    "arch",
    type=click.Choice(["amd64", "i386", "armhf", "arm64", "s390x", "ppc64"]),
    required=True,
    help="Architecture to filter on",
)
@click.option(
    "-k", "--kernel-version", "kver", required=True, help="Kernel version to filter on"
)
@click.option("-d", "--dest-dir", "dest_dir", default=os.getcwd())
###############################################################
def main(arch, kver, dest_dir):

    app = Kca()

    r = app.session.get(app.k_url).text.encode("ascii", "ignore").decode("utf-8")
